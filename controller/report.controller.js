    ...
    try {
        ...
        if (userid.toString() !== process.env.ADMIN_ID) {
            ...
            return res.status(403).json({
                success: false,
                message: "Unauthorized only admin can see reports"
            });
        ...
        const reports = await reportusermodel
            .find()
            .populate("reporter", "profilename avatar")
...
export const validatereport = async (req, res) => {
    ...
    try {
        ...
        if (!reporid || !status) {
            ...
            return res.status(400).json({
                success: false,
                message: "Report ID and status is required"
            });
        ...
        const report = await reportusermodel
            .findById(reporid)
            .populate("reporteduser", "profilename")
...
`;
        ...
        const mail = await reportusermail(
            reporteduseremail,
            message
        );
...
export const validatebugs = async (req, res) => {
    ...
    try {
        ...
        const report = await reportbugmodel
