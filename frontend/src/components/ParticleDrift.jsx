...
/**
 * Particle Drift (port of the Originkit WebGL component, JS + full-page
 * pointer tracking). Falling particles with proximity lines; particles
 * near the pointer light up in the accent colour and link to it.
 * Rendered as a transparent fixed layer behind the content (dark mode).
 */
...
export default function ParticleDrift({
  ...
  vRef.current = {
    base: baseColor,
    accent: accentColor,
    density: Math.round(Math.min(400, Math.max(10, density))),
    dotSize: Math.min(24, Math.max(1, dotSize)),
    speed: Math.min(100, Math.max(0, speed)) / 50,
    direction: Math.min(360, Math.max(0, direction)),
    hover: Math.min(200, Math.max(0, hover)) / 100,
    linkDistance: Math.min(400, Math.max(0, linkDistance)),
    linkThickness: Math.min(8, Math.max(0.5, linkThickness)),
  };
  ...
  useEffect(() => {
    ...
    const render = (now) => {
      ...
      const cb = parseColor(v.base, [0.612, 0.639, 0.686]);
      const ca = parseColor(v.accent, [0.94, 0.7, 0.24]);
    ...
    // track the pointer over the whole page, not just the canvas
    const track = (e) => {
      const r = canvas.getBoundingClientRect();
      if (r.width <= 0 || r.height <= 0) return;
      ptr.x = ((e.clientX - r.left) / r.width) * (canvas.clientWidth || r.width);
      ptr.y = ((e.clientY - r.top) / r.height) * (canvas.clientHeight || r.height);
    };
    const onLeave = () => {
      ptr.x = -10000;
      ptr.y = -10000;
  ...
  return (
    <div className="drift-host" aria-hidden="true">
      <canvas
        ref={canvasRef}
